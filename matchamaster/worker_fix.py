...
async def process_row(page, row):
    ...
    # 4b) sudah pernah submit? Klik cancel terlebih dahulu
    try:
        if await page.locator(SEL["cancel_submit"]).is_visible():
            logger.info(f"[{idsbr}] 🔄 Tombol Cancel Submit terlihat, klik cancel terlebih dahulu")
            await page.locator(SEL["cancel_submit"]).click()
            await page.wait_for_timeout(1000)  # Tunggu sebentar setelah cancel
            await handle_any_swal(page)  # Handle popup jika ada
            logger.info(f"[{idsbr}] ✅ Cancel submit berhasil, lanjut isi data dari database")
    except Exception as e:
        logger.warning(f"[{idsbr}] ⚠️ Gagal klik cancel submit: {e}")
        pass
